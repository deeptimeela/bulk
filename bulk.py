...
class bulk_tool_change:
  ...
  def changenameFile(self,filenames,changename,i):
            filename_without_ext = os.path.splitext(filenames)[0]
            extension = os.path.splitext(filenames)[1]
            new_file_name = changename + str(i)
            new_file_proper = new_file_name+extension
            #print(def stringFun(self):
            changename=input("enter the word you want to name of the file to")
            ...
            i = 1 
            for filenames in os.listdir(self.path) :
                  if((self.path + "/" + filenames) == sys.argv[0]):
                        continue 
                  self.changenameFile(filenames,changename,i)
                  i += 1 
            os.rename(os.path.join(self.path,filenames),os.path.join(self.path,new_file_proper))
      



  def change_for_x(self,sorted_file):
     changename=input("enter the word you want to name of the file to")
     ...
     if(('*' in changename)or('<' in changename)or('>' in changename)or('?'in changename)or('/'in changename)or('\\' in changename)or('|' in changename)or(':' in changename)):
                  print("Filename shouldnot contain *<>?/\|:")
     number_of_files=int(input("enter the number of files you want to rename"))
     i=1
     for file_name in sorted_file:
          if(i > number_of_files) : 
                  break 
          self.changenameFile(file_name,changename,i)
          i=i+1
